...
`server/vocab.py` picks each page's NEW general-English words and gives each a
concise Arabic meaning; this module is the one renderer and the one inserter of
those entries, shared by every artifact they ride in — the mono result
(server/pipeline.py), the recomposed duals (server/compose.py) and the
interlinear overlay (server/interlinear.py). A page's vocab page goes
...
DESIGN: deliberately LIGHTER than the «شرح المصطلحات» cards
(server/glossary_pages.py) — this is a quick word list, not a set of
explanations. Same palette family (the #1d4ed8 accent on the English word,
#111827 body, #64748b muted), but rows instead of cards: a small muted title
(«كلمات هذه الصفحة»), then tight RTL rows of bold English word — Arabic
meaning — optional muted note, with a soft #F1F5F9 stripe on alternate rows.
More than six entries flow into two columns (right column first — the page is
RTL); a page that still overflows continues onto a second inserted page (rare:
the extraction caps a page at 12 words).

TEXT and FONTS are the glossary renderer's own machinery, shared rather than
copied: `PageFonts` subsets the regular + bold GoNotoKurrent faces once per
document, `measure` runs the Story.place pass that decides row heights and
page breaks, and every caller saves with `garbage=4` for the same
per-box-font-copy reason.
"""
...
from server.glossary_pages import PageFonts
from server.glossary_pages import _escape
from server.glossary_pages import measure

logger = logging.getLogger("doctranslate.vocab_pages")

# The glossary palette family, applied lighter.
...
# out of one request.
_MAX_ROWS = 12    # mirrors vocab.MAX_PER_PAGE
_MAX_TOTAL = 150  # mirrors vocab.MAX_TOTAL
