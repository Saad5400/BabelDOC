...
def test_a_page_is_capped_at_twelve_words():
    entries = [{"w": f"word{chr(97 + i)}", "ar": "معنى"} for i in range(20)]
    client = _FakeClient([_reply({"0": entries})])

    result = vocab.extract_vocab(_sidecar({0: "text"}), client=client)

    assert len(result["0"]) == vocab.MAX_PER_PAGE == 12
    assert result["0"] == entries[:12]


def test_the_document_is_capped_at_one_hundred_fifty_words():
    ...
    result = vocab.extract_vocab(
        _sidecar(dict.fromkeys(range(20), "text")), client=client)

    assert sum(len(v) for v in result.values()) == vocab.MAX_TOTAL == 150
    # Ascending pages: the earliest words survive the cap.
    assert "0" in result and "19" not in result  # noqa: PT018
